...
// The UserBalanceLedgerFunc type is an adapter to allow the use of ordinary function as a Querier.
type UserBalanceLedgerFunc func(context.Context, *ent.UserBalanceLedgerQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f UserBalanceLedgerFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.UserBalanceLedgerQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.UserBalanceLedgerQuery", q)
}

// The TraverseUserBalanceLedger type is an adapter to allow the use of ordinary function as Traverser.
type TraverseUserBalanceLedger func(context.Context, *ent.UserBalanceLedgerQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseUserBalanceLedger) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseUserBalanceLedger) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.UserBalanceLedgerQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.UserBalanceLedgerQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.UserBalanceLedgerQuery:
		return &query[*ent.UserBalanceLedgerQuery, predicate.UserBalanceLedger, userbalanceledger.OrderOption]{typ: ent.TypeUserBalanceLedger, tq: q}, nil
