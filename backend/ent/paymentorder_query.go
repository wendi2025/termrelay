...
func (_q *PaymentOrderQuery) QueryBalanceLedger() *UserBalanceLedgerQuery {
...
func (_q *PaymentOrderQuery) WithBalanceLedger(opts ...func(*UserBalanceLedgerQuery)) *PaymentOrderQuery {
	query := (&UserBalanceLedgerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) loadBalanceLedger(ctx context.Context, query *UserBalanceLedgerQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *UserBalanceLedger)) error {
	...
	query.Where(predicate.UserBalanceLedger(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.BalanceLedgerColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
