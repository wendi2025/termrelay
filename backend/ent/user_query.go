...
type UserQuery struct {
	...
	withBalanceLedger         *UserBalanceLedgerQuery
...
func (_q *UserQuery) QueryBalanceLedger() *UserBalanceLedgerQuery {
	query := (&UserBalanceLedgerClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) WithBalanceLedger(opts ...func(*UserBalanceLedgerQuery)) *UserQuery {
	query := (&UserBalanceLedgerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBalanceLedger; query != nil {
		if err := _q.loadBalanceLedger(ctx, query, nodes,
			func(n *User) { n.Edges.BalanceLedger = []*UserBalanceLedger{} },
			func(n *User, e *UserBalanceLedger) { n.Edges.BalanceLedger = append(n.Edges.BalanceLedger, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadBalanceLedger(ctx context.Context, query *UserBalanceLedgerQuery, nodes []*User, init func(*User), assign func(*User, *UserBalanceLedger)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userbalanceledger.FieldUserID)
	}
	query.Where(predicate.UserBalanceLedger(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BalanceLedgerColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
