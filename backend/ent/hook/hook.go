...
// The UserBalanceLedgerFunc type is an adapter to allow the use of ordinary
// function as UserBalanceLedger mutator.
type UserBalanceLedgerFunc func(context.Context, *ent.UserBalanceLedgerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserBalanceLedgerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserBalanceLedgerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserBalanceLedgerMutation", m)
}
